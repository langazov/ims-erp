<script lang="ts">
  ...
  // Lucide-style SVG icon paths (inline for zero-dep), pre-generated at module level
  ...
  const FALLBACK_ICON_PATHS = `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M5 12h14M12 5l7 7-7 7"/>`;

  // Pre-generate all SVG strings once at module level to avoid repeated string concatenation
  const ICON_SVG_CACHE: Record<string, string> = Object.fromEntries(
    Object.entries(ICON_PATHS).map(([key, paths]) => [
      key,
      `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="nav-icon-svg">${paths}</svg>`
    ])
  );
  const FALLBACK_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="nav-icon-svg">${FALLBACK_ICON_PATHS}</svg>`;

  function getIconSvg(name: string): string {
    return ICON_SVG_CACHE[name] ?? FALLBACK_ICON_SVG;
  }
